import { GameState, GemColor, DevelopmentCard, Player, GemCount } from './models';
...
export const canAffordCard = (player: Player, card: DevelopmentCard): boolean => {
  let missingTokens = 0;
...
export const takeTokens = (state: GameState, tokens: GemColor[]): GameState => {
  if (state.turnPhase !== 'MainAction') throw new GameError("Not in main action phase.");
  if (tokens.length === 0) throw new GameError("Must select tokens to take.");
  if (tokens.includes(GemColor.Gold)) throw new GameError("Cannot take Gold tokens directly.");
  ...
  if (tokens.length === 2 && uniqueColors === 1) {
    ...
    if (newState.bank[color] < 4) {
      throw new GameError(`Must be at least 4 ${color} tokens in the bank to take two.`);
    }
    ...
    throw new GameError("Invalid token selection. Take 3 different or 2 of the same.");
  }
...
export const discardTokens = (state: GameState, tokensToDiscard: GemColor[]): GameState => {
  const newState = cloneState(state);
  const player = newState.players[newState.currentPlayerIndex];
  ...
  if (currentTotal - tokensToDiscard.length !== 10) {
    throw new GameError(`You must discard exactly ${currentTotal - 10} tokens.`);
  }
...
export const purchaseCard = (state: GameState, cardId: string): GameState => {
  ...
  for (const l of [1, 2, 3] as const) {
    const idx = newState.boardCards[`level${l}`].findIndex(c => c.id === cardId);
    if (idx !== -1) {
  ...
  if (!canAffordCard(player, cardToBuy)) throw new GameError("Cannot afford this card.");
  ...
  for (const [colorStr, cost] of Object.entries(cardToBuy.cost)) {
    ...
    if (available < cost) {
      ...
      const tokensToPay = Math.max(0, cost - (player.currentBonuses[color] || 0));
  ...
  player.currentBonuses[cardToBuy.colorBonus] = (player.currentBonuses[cardToBuy.colorBonus] || 0) + 1;

  if (source === 'board' && level) {
    const deck = newState.decks[`level${level}`];
    const boardLevel = newState.boardCards[`level${level}`];
    const idx = boardLevel.findIndex(c => c.id === cardId);
    if (deck.length > 0) {
      boardLevel[idx] = deck.shift()!;
    } else {
      boardLevel.splice(idx, 1);
    }
...
export const reserveCard = (state: GameState, cardId: string): GameState => {
  if (state.turnPhase !== 'MainAction') throw new GameError("Not in main action phase.");
  const newState = cloneState(state);
  const player = newState.players[newState.currentPlayerIndex];
  ...
  if (!cardToReserve || !level) throw new GameError("Card not found on board.");
...
export const reserveCardFromDeck = (state: GameState, level: 1 | 2 | 3): GameState => {
  if (state.turnPhase !== 'MainAction') throw new GameError("Not in main action phase.");
  const newState = cloneState(state);
  const player = newState.players[newState.currentPlayerIndex];
  ...
  // Take the actual top card from the deck
